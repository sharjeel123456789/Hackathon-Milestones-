document.addEventListener("DOMContentLoaded", () => {
  ...
  const addField = (container: HTMLElement, template: string) => {
      const lastEntry = container.lastElementChild as HTMLDivElement;
  ...
  form.addEventListener("submit", (e) => {
      ...
      const educations = Array.from(document.querySelectorAll("#education-section .education-entry"))
          .map(entry => {
      ...
      const experiences = Array.from(document.querySelectorAll("#experience-section .experience-entry"))
          .map(entry => {
